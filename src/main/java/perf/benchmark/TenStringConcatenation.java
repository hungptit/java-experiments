...
@Fork(1)
@State(Scope.Thread)
public class TenStringConcatenation {
  private String s0;
  private String s1;
  private String s2;
  private String s3;
  ...
  public void testStringFormat(Blackhole bh) {
    var combined = String.format("%s%s%s%s%s%s%s%s%s%s", s0, s1, s2, s3, s4, s5,
                                 s6, s7, s8, s9);
    bh.consume(combined);
  }
  ...
  public void testPlus(Blackhole bh) {
    var combined = s0 + s1 + s2 + s3 + s4 + s5 + s6 + s7 + s8 + s9;
    bh.consume(combined);
  }
  ...
  public void testStringJoin(Blackhole bh) {
    var combined = String.join("", s0, s1, s2, s3, s4, s5, s6, s7, s8, s9);
    bh.consume(combined);
  }

  @Benchmark
  public void testStringConcat(Blackhole bh) {
    var combined = s0.concat(s1)
                       .concat(s2)
                       .concat(s3)
                       .concat(s4)
                       .concat(s5)
  ...
  public void testStringUtilsJoin(Blackhole bh) {
    var combined = StringUtils.join(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9);
    bh.consume(combined);
  }

  @Benchmark
  public void testGuavaJoiner(Blackhole bh) {
    var combined = Joiner.on("").join(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9);
    bh.consume(combined);
  }

  @Benchmark
  public void testStreamJoining(Blackhole bh) {
    var combined = Arrays.asList(s0, s1, s2, s3, s4, s5, s6, s7, s8, s9)
                       .stream()
