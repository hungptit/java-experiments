...
public class FiveStringConcatenation {
  ...
  public void testStringFormat(Blackhole bh) {
    var combined = String.format("%s%s%s%s%s", s0, s1, s2, s3, s4);
    bh.consume(combined);
  }

  @Benchmark
  public void testPlus(Blackhole bh) {
    var combined = s0 + s1 + s2 + s3 + s4;
    bh.consume(combined);
  }
  ...
  public void testStringBuilder(Blackhole bh) {
    var sb =
        new StringBuilder().append(s0).append(s1).append(s2).append(s3).append(
            s4);
  ...
  public void testStringBuffer(Blackhole bh) {
    var sb =
        new StringBuffer().append(s0).append(s1).append(s2).append(s3).append(
            s4);
    bh.consume(sb.toString());
  }
  ...
  public void testStringJoin(Blackhole bh) {
    var combined = String.join("", s0, s1, s2, s3, s4);
    bh.consume(combined);
  }

  @Benchmark
  public void testStringConcat(Blackhole bh) {
    var combined = s0.concat(s1).concat(s2).concat(s3).concat(s4);
    bh.consume(combined);
  }

  @Benchmark
  public void testStringUtilsJoin(Blackhole bh) {
    var combined = StringUtils.join(s0, s1, s2, s3, s4);
    bh.consume(combined);
  }

  @Benchmark
  public void testGuavaJoiner(Blackhole bh) {
    var combined = Joiner.on("").join(s0, s1, s2, s3, s4);
    bh.consume(combined);
  }

  @Benchmark
  public void testStreamJoining(Blackhole bh) {
    var combined = Arrays.asList(s0, s1, s2, s3, s4)
                       .stream()
