...
public class TwoStringConcatenation {
  ...
  public void testStringFormat(Blackhole bh) {
    var combined = String.format("%s%s", s0, s1);
    bh.consume(combined);
  }
  ...
  public void testStringBuilder(Blackhole bh) {
    var sb = new StringBuilder().append(s0).append(s1);
    bh.consume(sb.toString());
  }

  @Benchmark
  public void testStringBuffer(Blackhole bh) {
    var sb = new StringBuffer().append(s0).append(s1);
    bh.consume(sb.toString());
  }

  @Benchmark
  public void testStringJoin(Blackhole bh) {
    var combined = String.join("", s0, s1);
    bh.consume(combined);
  }
  ...
  public void testStringUtilsJoin(Blackhole bh) {
    var combined = StringUtils.join(s0, s1);
    bh.consume(combined);
  }

  @Benchmark
  public void testGuavaJoiner(Blackhole bh) {
    var combined = Joiner.on("").join(s0, s1);
    bh.consume(combined);
  }

  @Benchmark
  public void testStreamJoining(Blackhole bh) {
    var combined = Arrays.asList(s0, s1).stream().collect(Collectors.joining());
    bh.consume(combined);
  }
}
